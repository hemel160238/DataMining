...
def probability_correlation_contrast_uniformity_homogeneity_entropy(image_path):
    img = Image.open(image_path).convert('L')
    image_array = np.array(img)

    out_matrix = greycomatrix(image_array,[1], [0], levels=256, symmetric=True, normed=True)

    maximum_probability = np.amax(out_matrix)
    correlation = greycoprops(out_matrix, 'correlation')[0][0]
    contrast = greycoprops(out_matrix, 'contrast')[0][0]
    energy = greycoprops(out_matrix, 'energy')[0][0]
    homogeneity = greycoprops(out_matrix, 'homogeneity')[0][0]
    entropy = skimage.measure.shannon_entropy(out_matrix)

    results = [maximum_probability, correlation, contrast, energy, homogeneity, entropy]
...
def WriteXL(dataList):
    ...
    xsheet.write_row(0, 0, ['image_name', 'max_probability', 'correlation', 'contrast', 'energy', 'homogeneity', 'entropy'])
...
def savefeature():
    ...
    for imagePath in imagefiles:
        ...
        six_image_features = probability_correlation_contrast_uniformity_homogeneity_entropy(full_path)
        dataList.append([full_path, six_image_features[0], six_image_features[1], six_image_features[2], six_image_features[3], six_image_features[4], six_image_features[5]])
...
def claculateCanberreaDist(point1, point2):
    max_probability_p1 = point1[0]
    max_probability_p2 = point2[0]

    correlation_p1 = point1[1]
    correlation_p2 = point2[1]

    contrast_p1 = point1[2]
    contrast_p2 = point2[2]

    energy_p1 = point1[3]
    energy_p2 = point2[3]

    homogeneity_p1 = point1[4]
    homogeneity_p2 = point2[4]

    entropy_p1 = point1[5]
    entropy_p2 = point2[5]

    c_distance = ((abs(max_probability_p1 - max_probability_p2) / (
            abs(max_probability_p1) + abs(max_probability_p2))) +
                  (abs(correlation_p1 - correlation_p2) / (abs(correlation_p1) + abs(correlation_p2))) +
                  (abs(contrast_p1 - contrast_p2) / (abs(contrast_p1) + abs(contrast_p2))) +
                  (abs(energy_p1 - energy_p2) / (abs(energy_p1) + abs(energy_p2))) +
                  (abs(homogeneity_p1 - homogeneity_p2) / (abs(homogeneity_p1) + abs(homogeneity_p2))) +
                  (abs(entropy_p1 - entropy_p2) / (abs(entropy_p1) + abs(entropy_p2))))
...
def recognise():
    ...
    test_image = probability_correlation_contrast_uniformity_homogeneity_entropy(query_image)

    for unit_image in npArray:
        durotto = claculateCanberreaDist(unit_image[1:], test_image)
